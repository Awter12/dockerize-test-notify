...
pipeline {
    ...
    post {
        success {
            slackSend(channel: '#dockerize-test-notify', color: 'good', message: "Build & Deploy succeeded: ${env.JOB_NAME} #${env.BUILD_NUMBER}")
        }
        failure {
            slackSend(channel: '#dockerize-test-notify', color: 'danger', message: "Build & Deploy failed: ${env.JOB_NAME} #${env.BUILD_NUMBER}")
        }
        always {
            sh 'docker logout'
        }
